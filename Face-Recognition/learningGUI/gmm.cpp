...
bool Gmm::Recogniser(const vector<vector<double>>& melCepData, size_t frameCount)
{
    double prob;
    double probMax = -999999.5;
    string name;
    ...
    while(it != itEnd)
    {
        ...
        cout<<"prob"<<prob<<endl;
        if(probMax<=prob)//越大越好
        {
            if((prob-probMax)<3000)
                return false;
            probMax = prob;
            name = it->first;
        }
        ...
        {
            if((probMax-prob)<3000)
                return false;
        }
        ++it;
    }
    cout<<"probMax"<<probMax<<name<<endl;

    normProb.clear();
    mixedProb.clear();
    if(probMax>gmmcorrect)
        return true;
    return false;
